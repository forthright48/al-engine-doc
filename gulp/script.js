...
module.exports = function(gulp) {
  ...
  gulp.task('script', function(done) {
    ...
    filePathArray.forEach(function(filePath) {
      let destPath = path.relative(path.join(config.rootPath, 'src'), filePath);
      destPath = path.join('./public', destPath);
      const destDir = path.dirname(destPath);

      // Get Modified Time
      /*const mtimeSource = fs.statSync(path.join(config.rootPath, filePath)).mtime;
      let mtimeDest;
      try {
        mtimeDest = fs.statSync(path.join(config.rootPath, destPath)).mtime;
      } catch (e) {
